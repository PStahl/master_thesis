...
In this paper we assume a distributed cloud system, with heterogeneous hardware and interconnected nodes as shown in figure~\ref{fig:computational_environment}. We refer to a computational resource as a \emph{node}.
...
We will ensure reliability based on task replication, using active replication, where each replica receives the same input, and performs the same calculations. Figure~\ref{fig:app_model_replication} shows how the application looks like after replication of task $A$.
...
%constant failure rate Poisson = Homogeneous Poisson process
%Non-Homogeneous Poisson - variable failure rate [System Reliability Concepts - Springer] <- Bok

However, when a system first is put into action (use other words), no knowledge about the failure rates of the system is known. Therefore, when the system is set up, we do start with assuming constant failure rates.
...
The reliability level over time was measured by continuously counting the number of replicas running, and monitoring the reliability of the nodes.
...
%MOVE?
In case of link failure two nodes might get unavailable of each other. Each node will assume that the other has failed and replicates new replicas until required reliability is achieved. Therefore, from a system perspective, there is a unnecessary high reliability. Since our system in a dynamic way keeps track of the total reliability during runtime and kills unnecessary replicas this type of link failure isn't a problem?
...
\bibitem{surveyRelPrediction}
	...
	Software & Systems Modeling, 7:49-65,
	February 2008

\bibitem{relDistApplications}
	C. A. Tănasie, S. Vîntutis, A. Grigorivici,
	\epmh{Reliability in Distributed Software Applications},
