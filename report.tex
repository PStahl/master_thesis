...
The reliability of a process is measured by the probability that the task is executed within a specified amount of time without experiencing a hardware, software or a network failure during it's time of execution.
\end{definition}
...
%---------- -------------------------DECIDE WHICH FACTOR WE SHOULD USE-----------------------------------------


\begin{equation} \label{eq:overall_reliability}
R(t) = R_{HW}(t) \cdot R_{SW}(t) \cdot R_{Network}(t)
\end{equation}
where $R_{SW}(t)$ and $R_{Network}(t)$ is the probability that the software respectively the network is free from failures during time $t$. 
...
Thereafter we began our literature study of today's situation, as seen in \ref{sec:related_work} there is a huge interest in this subject...etc
Philip implemented the replication functionality in Calvin and Jonatan wrote a first model to the task scheduling/supervisor mechanism. 
