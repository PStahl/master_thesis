...
A dynamic and adaptive algorithm, which dynamically varies the number of replicas depending on system load is presented in~\cite{adaptiveCheckPointAndRep}. The proposed algorithm does not however ensure a certain reliability, instead it reduces the number of replicas during peak hours, in order to reduce system load. Since the reliability of system decreases during higher load~\cite{studyOfFailures} \cite{studyOfFailures} \cite{implicationsOfFailures}, one should increase the number of replicas to keep the desired level of reliability.
...
The framework is not limited to a specific type of distributed environment, not the reliability model used. The reliability model and scheduling algorithm is easily replaced to consider more parameters, or to include load-balancing.
...
In a dynamic environment usually heterogeneous resources are installed~\cite{compStudyLoadAndCloud}. In this scenario prior knowledge isn't enough since the requirements of the user can change during run time. Therefore run time statistics is collected and taken into account. The dynamic environment is difficult to simulate but there are algorithms which easily adopt to run time changes in load. Once the system has been put into place, resources can be dynamically added or removed.
...
The crash failure model is quite alike the fail-stop model with the difference that nodes do not automatically detect the failure of a node~\cite{faultTolerantFundamentals} \cite{adaptiveAgentReplication}.

\subsection{Failure distribution} \label{subsec:background_failure_distribution}
Models describing the nature of failures in distributed computing environments, are usually based on certain assumptions, and are usually only valid under those assumptions. Commonly made assumptions about failures are \cite{relModelDistSimSystem} \cite{relModelAnalysis} \cite{cloudServiceRel} \cite{studyServiceRel} \cite{hierarchicalRelModeling} \cite{selfAdaptRel}:
\begin{itemize}
...
For reliability modelling for the grid, it is common to also assume a fully reliable Resource Management System (RMS)~\cite{relAndPerfGridServices} \cite{relGridServicePredConstraint}, which is an non-negligible-able assumption since the RMS in this case in fact is a single point of failure.
...
Reliability of a system highly depends on how the system is used~\cite{surveyRelPrediction}. In order to determine the reliability of a system, one need to take all factors affecting the reliability into account~\cite{surveyReliabilityDistr}. However, including all factors if unfeasible. In \cite{factorsAffectingRel}, 32 factors affecting the reliability of software are listed, excluding environmental factors such as hardware and link failure. Other environmental conditions affecting reliability include the amount of data being transmitted, available bandwidth and operation time~\cite{cloudServiceRel} \cite{hierarchicalRelModeling}.
...
Furthermore, we assume the replicated tasks/services are deterministic and always produces the correct result for a given input. This allows us to focus on the reliability of producing a result and not lose any data, instead of the reliability of producing the correct result as done in~\cite{selfAdaptRel} \cite{dynAdaptRepl} \cite{relModelWebServices}. As mentioned before this can be solved by extending the model with consensus such as majority voting. Due to that we don't gain any extra useful information we have chosen to leave this to future work.
...
%TODO mention that we focus on System related failures (hardware failures).
%TODO referr back to commonly made assumptions, fully reliable network, etc.
...
We assume failures follow a Poisson process, as this seems to be widely accepted in the research community~\cite{experimentalFailureAssessment}. The Poisson distribution is defined as follows:
...
where $F_1$ \dots $F_m$ are the failure probability for the $m$ resources on which the $n$ replicas are running. Using this model, reliability is only increased through replication if the replicas are scheduled on separate nodes.
...
%TODO .... we assume very low latency and thereby negligible time to send the heartbeat.... otherwise it affects the time to detect node failure
...
When getting available nodes, sorted by reliability with the preferred nodes first, it is important to consider the current nodes. Since we consider only node failures, and assume statistically independent failures, it is important not to place two replicas on the same node. But, it is also important if one were to take more parameters into account. For example, one may not want to have several replicas in the same rack, as they would all fail if the rack's switch fails.
...
\begin{algorithm} 
	...
	\Do
		...
		\State
		\If {$R_{most\_reliable}(t) > R_{least\_reliable}(t)$}
			...
			\State $most\_reliable\gets nodes.pop$
		\EndIf
	\doWhile {$R_{most\_reliable}(t) > R_{least\_reliable}(t)$}
	\EndProcedure
	\State
	...
	\Do 
		\State $least\_reliable\gets current\_nodes.pop$
		\If {\Call{reliability}{$current\_nodes$}\ >\ $\lambda$}
			\State $replica\gets $\Call{replica at}{$least\_reliable$}
			\State
			\Call{delete replica from node}{$replica$, $least\_reliable$}
		\EndIf
	\doWhile {\Call{reliability}{$current\_nodes$}\ >\ $\lambda$}
	\EndProcedure
	\end{algorithmic}
\end{algorithm}

The first part of~\cref{alg:optimization} make sure the most reliable nodes are used, by moving the replica on the least reliable node to the most reliable node available as long as more reliable nodes are available. The second part make sure no unnecessary replica is used, by deleting the replica on the least reliable node as long as the required reliability is still met.

\Cref{alg:optimization} make sure no unnecessary replicas are used. However, moving replicas to more reliable nodes may be unnecessary overhead if it does not affect the overall number of replicas needed. For example, moving from a node with a reliability of 0.999 to a node with reliability 0.9991 does not increase the overall reliability very much, and if it does not result in another replica can be deleted, it may be seem unnecessary.

\section{Monitoring} \label{sec:design_monitoring}
%TODO push pull strategy
...
%TODO what is the runtime dies? Should be change to handling actors on each runtime? And for a given replica, find all replicas, and chose the node with the highest ID?
