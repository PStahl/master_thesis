...
Throughout the whole master thesis work extensive literature studies has been made, first in order to find out what has already been done in the area and later in order to get valuable help in developing a reliability model.
...
Lastly, the system will by continously monitored in order to adapt the reliability model and the replicas needed as the properties of the system varies over time.
...
In this paper we assume a all resources are within the same cluster, with low latency connections between them. The nodes consist of heterogeneous hardware and interconnected nodes as shown in figure~\ref{fig:computational_environment}. We refer to a computational resource as a \emph{node}.
...
\caption{An application model where a producer transmits data to a task A, which transforms the data, and sends the result to a consumer. A may be seen as a task or service requiring a certain level of reliability.}\label{fig:app_model}
\end{figure}
...
Thereafter, the failure rate is calculated from the the replication time and the MTBF, which is based on the number of experienced failures and the time they occurred.

Furthermore, the time it takes to replicate a task is not known at deployment time. Before an actor has been replicated and the time to replicate it been measured we assume a constant replication time. The replication time is dynamic, and actor type dependent. Some actors may take longer time to replicate than others, why a system wide average may be far from correct for a given application.
...
\subsection{Expressing time t}
The time $t$ used in \ref{eq:desired_rel} can be expressed as the time it takes from that a failure happened, until a new replica is operational. We can therefore express t as
...
The reliability level over time was measured by continuously and monitoring the reliability of the nodes on which replicas where running.
...
\subsection{Node discovery time}


\subsection{Constant failure rate}
In this example, all eight nodes were given the same \emph{Mean-Time-Between-Failures}.

The MTBF for each node was a normal distribution with mean 30 seconds and standard deviation set to 5. With a MTBF equal to 30, the reliability for each node in this experiement was %TODO.

Furthermore, the desired reliability in this experiement was set to 0.999\%.
