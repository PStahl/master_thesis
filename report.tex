...
\begin{algorithm} 
	...
	\begin{algorithmic}[1]
	\State $n\gets current\_replicas$
	
	\State $nodes\gets available\ nodes$\Comment{sorted after reliability in descending order}
...
\Cref{alg:scheduling} is designed to use the reliability model in a plugin fashion. When sorting the nodes, one can easily take other parameters into account, such as the load of the nodes.
%TODO We actaully take the load into account now
...
\begin{algorithm} 
	...
	\begin{algorithmic}[1]
	\Do 
		\State $current\gets current\_nodes$\Comment{sorted after reliability in ascending order}
		\State $nodes\gets available\ nodes$\Comment{sorted after reliability in descending order}
		\State $least\_reliable\gets current\_nodes.pop$
		\State $most\_reliable\gets nodes.pop$
		\State
		\If {\Call{reliability}{$most\_reliable$}$\ >\ $\Call{reliability}{$least\_reliable$}}
			\State $replica\gets $\Call{$replica\_at$}{$least\_reliable$}
			\State
			...
			\State $most\_reliable\gets nodes.pop$
		\EndIf
	\doWhile {\Call{reliability}{$most\_reliable$}$\ >\ $\Call{reliability}{$least\_reliable$}}
	\State
	
	\Do 
		\State $current\gets current\_nodes$
		\Comment{sorted after reliability in ascending order}
		\State $least\_reliable\gets current\_nodes.pop$
		\If {\Call{reliability}{$current$}\ >\ required}
			\State $replica\gets $\Call{$replica\_at$}{$least\_reliable$}
			\State
			\Call{delete replica from node}{$replica$, $least\_reliable$}
		\EndIf
	\doWhile {\Call{reliability}{$current$}\ >\ required}
	\end{algorithmic}
\end{algorithm}
...
The hearbeat system described in~\cref{subsec:heartbeats}, was implemented using actors. Each runtime creates a heartbeat actor when its started. The heartbeat actor listens for heartbeats as well as periodically sends heartbeats to other runtimes.
