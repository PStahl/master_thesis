...
The time to detect a failed node depends on the frequency of the heartbeats. In our model, heartbeats are sent to all nodes in the system every 200ms. A node is considered dead when no heartbeat is received for 500 ms, which thus is the time to detect a node failure.
...
\section{Computational environment}
We mentioned in ~\ref{ch:design}, we implemented our model using Calvin. In order to evaluate our model, we set up an execution environment consisting of 7 servers, and started a Calvin runtime on each server. One of these runtimes was stable, while each of the other runtimes were continously killed and restarted, simulating a node failure, after which it was restarted.
