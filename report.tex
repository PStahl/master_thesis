...
The interest in reliability in distributed systems has gain increased knowledge. Due to the uncertain heterogeneous environment of cloud and grid systems, increasing reliability is a complex task [TODO]. %TODO add reference
...
Approaches to improve reliability in Multi-Agent Systems (MAS) by the use of replication is presented in \cite{replicatingAgents} and \cite{adaptiveMASReplication}. While being adaptive to system state, the solution presented in \cite{replicatingAgents} still faces the problem of having a single point of failure due to a communication proxy. This problem is avoided in \cite{adaptiveMASReplication}, where a decentralized solution is proposed, where the number of replicas and their placement depends on the system state. %TODO need to read \cite{adaptiveMASReplication} again...
...
\\\\
Hardware failures do not usually have a constant rate of failure. Instead, hardware is less reliable and more likely to fail in the beginning and of its life-cycle \cite{surveyReliabilityDistr}. %TODO bath-tub curve
\\\\
Fault may propagate throughout the system, thereby affecting other components as well \cite{relGridSystems}.
\\\\
COPIED: failures is modeled well by a Weibull distribution  (Weibull shape parameter of 0.7–0.8) with decreasing hazard rate.  \cite{studyOfFailures}.
\\\\
...
\\\\
COPIED: “where failure rates vary significantly depending on a node’s workload… the front-end nodes, which run a more varied, interactive workload, exhibit a much higher failure rate than the other nodes “
 \cite{studyOfFailures}
\\\\
...
Reliability of a system highly depends on how the system is used \cite{surveyRelPrediction}.
