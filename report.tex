...
%TODO
Virtualization is a broad term of creating a virtual version of something, e.g. a server, a hard drive or a private network (VPN). The wins of virtualization is partly savings in hardware cost and space and partly in that the system becomes more dynamic. E.g. it is possible to virtualize a hundred servers on fifty physical servers since they usually have capacity left. Another con with virtualization is that it separates the software from the hardware which enables the possibility of moving a virtual machine from one physical server to another.
...
In this paper we assume that all resources are within the same cluster, with low latency connections between them. The nodes consist of homogeneous hardware and interconnected nodes as shown in figure~\ref{fig:computational_environment}. We refer to a computational resource as a \emph{node}. Due to that the hardware components are homogeneous, approximately the same behaviour in terms of failure, we will simulate that the behaviour is different in order to get more interesting results.
