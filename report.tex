...
The timeout time of 500 ms is an upper bound to how long it takes to detect a node failure. A node may fail just before sending a heartbeat, or directly after, which will affect the actual time it took to detect the failure. This corresponds to the best and worst case scenario. 
...
In the latter case, node A dies directly after sending a heartbeat, since it will no longer send heartbeats, that one will timeout, and the time to detect it will therefore be 500 ms, which is thus an upper bound of detecting node failures. See~\cref{appendix:figure} for an example.
...
\subsection{Using previously registered times}
The time $T_R$ can vary depending on whether or not the first selected node succeeds in replicating its actor, or if it dies before the replication is done. In the latter case, a new node is selected and asked to replicate its replica, further described in~\cref{sec:design_handling_failure}. This situation can theoretically continue until there is no more nodes holding a replica. One could argue that this should be taken into account in the reliability model. When the sending node does not receive a reply within a certain time, (e.g. set to twice the replication time), the request is assumed to have failed and the request is sent to another node. When this happens, the total time $R_{xxx}$ will be relatively high, as time was wasted sending the request to the first node. Instead of including this in the reliability model, a log-logistic distribution if fitted to the previous registered times. From this distribution, the 95th percentile value is used. For 95 percent of the cases, the time $R_{xxx}$ will actually be lower than this value.
