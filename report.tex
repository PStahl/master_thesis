...
\subsubsection*{Reliabilty definition}
...
\begin{definition} \label{def:replica_reliability}
...
\subsubsection*{Reliability models}
...
From \ref{eq:MTBF} a reliability function can be expressed as 
\begin{equation} \label{eq:resource_reliability}
R_{r}(t) = e^{-t/MTBF_{r}}
\end{equation}

Equation~\ref{eq:resource_reliability} expresses the probability that  resource $r$ will work for a time $t$.
...
\begin{equation} \label{eq:task_failure_probability}
R_{T}(t) = R_{replica_1}(t) \cdot R_{replica_2}(t) \cdots R_{replica_n}(t)
\end{equation}

where $R_{replica i}(t)$ is the reliability that replica $i$ is reliable during the time $t$, which it takes to replicate an actor. 

Assuming tasks themselves do not fail unless the resources they use fail, the reliability of a task replica is dependent on the reliability of the resources it uses. Limiting the node failure, either in terms of hardware failure or lost connectivity, equation~\ref{eq:task_failure_probability} can with equation~\ref{eq:resource_reliability} be re-written as

\begin{equation} \label{eq:task_failure_probability_2}
R_{T}(t) = R_{r_1}(t) \cdot R_{r_2}(t) \cdots R_{r_k}(t)
\end{equation}

where $R_{r_1} \cdots R_{r_k}$ are the resources on which the $n$ replicas are using.
\\\\
Given an task with $m-1$ replicas, the time $t$ it takes to replicate a task, and the failure probability $Pf_{k}$ during the time $t$ for each of the $m$ replicas, the task failure probability is
\begin{equation}
\label{eq:task_reliability}
Pf = \prod\limits_{k=1}^m Pf_{k}(t)
\end{equation}

Expressed in words, equation \ref{eq:task_reliability} is the probability that all replicas fail, during the time it takes to start a new one.
