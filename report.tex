...
\section{?} %I think a section would be good here
...
Choosing streaming services with deterministic processing\footnote{A determistic function always produces the same output given a certain input.}, allows us to avoid timing issues and cases when the consumer receives different results. However, since using active replication the consumer will receive a result from each replica and our model could therefore easily be extended with consensus algorithms such as those presented in \cite{selfAdaptRel} \cite{dynAdaptRepl} \cite{relModelWebServices} to determine whether or not the correct result was received.
%TODO (or ref to \ref{subsub:consensus})
...
As described in \ref{ch:approach} we limit our model to only consider node failures. Furthermore, we assume the replicated task/service is deterministic and always produces the correct result for a given input. 
