...
\begin{algorithm} 
	...
	\begin{algorithmic}[1]
	\Require{$current\_nodes$ is a list of nodes currently holding a replica}
	\Statex
	...
	\Do 
		\State $least\_reliable\gets current\_nodes.pop$
		\If {\Call{reliability}{$current\_nodes$}\ >\ required}
			\State $replica\gets $\Call{replica at}{$least\_reliable$}
			\State
			\Call{delete replica from node}{$replica$, $least\_reliable$}
		\EndIf
