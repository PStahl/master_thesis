...
The process of selecting where to place new replicas could be extended to consider whether or not the selected node has capacity for the new replica. The application author could, using some metric, define the resources needed for a task replica, and the system administrator assign each node a metric of available resources. These metrics could then be used to determine whether or not the available resources of a node is sufficient for a replica.
...
Furthermore, our model does not guarantee that the average of the actual reliability is above the required, it only aims at keeping the current reliability higher than the required. This allows for the user specifying various reliability requirements. One such requirement could be that the time the system is in a vulnerable state should not exceed some threshold. One possible way of keeping the average reliability above a predefined level is further described in \cref{subsec:future_extended_model}.
