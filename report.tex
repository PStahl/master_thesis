...
As mentioned, the servers used in the experiments had homogeneous hardware components, but varying failure rates were simulated by killing and restarting the runtimes with varying rates. Since a node must have failed at least twice in order to calculate its MTBF and thereby its reliability, a default value of 10 seconds was used in the experiments before two or more failure times had been registered.
...
In the following two experiments, the actor had only one out port and no in port. The state of the actor was in the first experiment incrementally increased by increasing the size of a local variable, while in the other it was increased by increasing the size of its output queue. The times measured was the time to serialize the state, send the state to the other runtime, de-serialize it, create and start the new actor, and send the reply. The state of the actor is only one part of the total replication message. The other part is static, and its size during the experiment was 349 bytes.
