...
As previously mentioned, reliability will be ensured by the use of replication. More specifically, active replication will be used where each replica receives the same input, and performs the same calculations. \autoref{fig:app_model_replication} shows how the application in \autoref{fig:app_model} looks after replicating task $A$ 4 times.
