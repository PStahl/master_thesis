...
\Date{\today}
\Advisor{Björn Landfeldt}%, \href{mailto:bjorn.landfeldt@eit.lth.se}{\texttt{bjorn.landfeldt@eit.lth.se}}}
...
For every heartbeat received, a timeout of $t_{\textrm{timeout}}$ seconds is set, and when it times out, the runtime from which the heartbeat was sent is assumed dead. On the other hand, when a heartbeat is received, all timeouts related to that runtime are canceled. When a timeout happens, \cref{alg:node_failure} is deployed and new replicas are created if needed to reach the required reliability, see~\cref{sec:design_handling_failure}.
...
\bibitem{relDistApplications}
	C. A. T?nasie, S. V�ntutis and A. Grigorivici,
