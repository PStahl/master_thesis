...
\subsubsection{Monitor} % Change name? Not to confuse with reliability monitor (i.e. the monitoring of each applications reliability)
With a monitoring system node failures can get detected quickly, if a node stops answering at heartbeats we can assume if has failed.
...
In order to validate our model, we conducted a set of experiments. The goal of the experiments were to show that our model dynamically ensures the desired level of reliability is met, dispite the event of node failures, and doing so in an optimal way by chosing the most reliable nodes and thereby the minimum number of replicas.
...
%TODO PREVIOUSLY chapter "Limitations"
