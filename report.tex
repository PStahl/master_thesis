...
%TODO also the node we're sending the lost node request to may fail!
...
%TODO mention that the actors preform deterministically calculations and therefore suits our application model of stream processing
...
%TODO text needs work
Since the actual time slept before killing and restarting the runtime is picked from a normal distribution, the actual MTBF may not be the assigned MTBF. The reason for using values form a normal distribution with mean equal to the given MTBF, instead of simply the given MTBF, was to simulate a more realistic situation. If all runtimes were given the same MTBF and slept exactly that time, they would all die at the same time if they were started at the same time. Furthermore, if one waited a small time between starting each runtime, two runtimes would never die at the same time. By picking values from a normal distribution, two or more runtimes may fail at the same time during the time of the experiments.
...
\Cref{figure:distribution_results} shows the four best fitted distributions, with log-logistic being the best.
