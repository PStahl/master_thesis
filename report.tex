...
\begin{itemize}
	\item Using Kademlia, data stored both locally and remotely, by distributing the data to other nodes in the system. The data is therefore available despite one of the storing nodes failing. DHT will not be further explained in this thesis, instead we refer to~\cite{kademlia} for further information about DHT and Kademlia especially. 
	\item When using the proxy storage, one runtime acts as a storage node, and all data is distributed to that runtime. All request for data is also sent to the runtime acting as proxy storage. However, the proxy storage introduces a single point of failure. %TODO mention some pros, eaiser when testing?
\end{itemize}
...
As mentioned in~\cref{design:calvin_storage}, Calvin supports both using the Kademlia implementation of DHT, and a proxy storage. We experienced the DHT to be very unstable, and not suitable for our experiments, were nodes were frequently killed and restarted, sometimes with a MTBF of 10 seconds. Therefore, the a storage proxy is used in the experiments. This introduce a single-point-of-failure, but the runtime acting as a proxy storage was never killed during the experiments. In a more realistic setting, nodes are unlikely to fail as often as every 10 seconds, and the use of DHT could in such a case be more suitable.
