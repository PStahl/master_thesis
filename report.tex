...
Cloud systems often consists of heterogeneous hardware and therefore ensuring a certain level of reliability is a complex task since any of the components may fail at any time. 

Guaranteeing properties such as high reliability, raises complexity to the resource allocation decisions, and fault-tolerance mechanisms in highly dynamic and complex distributed systems. For cloud service providers, it is necessary that this increased complexity is taken care of without putting extra burden on the user. The system must therefore ensure these properties in a seamless way.

As computational work is distributed across multiple resources, the overall reliability of the application decreases. To cope with this issue, a fault-tolerant design or error handling mechanism need to be in place. This is of particular interest for cloud service providers, as users often desire a certain level of reliability.

Sometimes vendors, for example carrier providers, are even obliged by law to achieve a certain level of availability or reliability. In these cases, one may need to sacrifice other quality aspects such as latency and resource usage. By using static and dynamic analysis of the infrastructure and the mean-time-to-failure for the given resources, a statistical model can be used in order to verify statistically that the desired level is reached.

Reliability can be increased by cloning application tasks, where all clones produce the same output. This of course leads to redundancy but also allows for an easy error check. If the execution of one clone stops or the link between a clone and the receiver is broken this can easily be detected at the receiver side and the scheduler mechanism can automatically find a proper location for a new clone. This allows for continuing execution of the application even in case of e.g. hardware failure.

Drawbacks of simply replicating a task n times, is that the resources needed increases. Furthermore, since only the work of one task is needed, a lot of computational resources is thus wasted. Dynamic analysis of the system could help in determining on which resources one should assign the tasks to for optimal resource usage and load balancing.
...
\section{Method}
we decided that our approach in this master thesis should be to use an action research oriented methodology. That means that we first studied today's situation from literature and with input from our supervisor Björn Landfeldt. Therefrom we could state the problem which was to be solved.
The second step was to suggest a proper solution and implement it. Here we got input from our supervisor and literature. The third and most important step was to evaluate the solution in an objective way. 

Same experimenting aspects also...
...
%Some conclusion regarding what def we will use.

To fully understand the above definitions we give a brief definition of the three terms failure, fault and error. 
\begin{itemize}
\item Failure: Is defined as the event, e.g. when the software produces the wrong result or when a server crashes. I.e. the inability to perform as required
\item Fault: A fault is a the state of software. It is also called defect/bug.
...
There are a number of different strategies for replication a task:
\begin{itemize}
...
Load balancing is often divided into two categorize, namely static and dynamic algorithms. The difference is that the dynamic algorithm takes into account the nodes previous states and performance whilst the static doesn't. The static load balancing simply looks at things like processing power and available memory at the moment which might lead to the disadvantage that the selected node gets overloaded \cite{perfAnalysisLoadCloud}

A dynamic load balancing can work in two ways, either distributed or non-distributed. In the distributed case the load balancing algorithm is run on all the nodes and the task of load  balancing is shared among them. This implies that each node has to communicate with all the others, effecting the overall performance of the network. % true also for static load balancing?
...
\item Local Queue Algorithm - 

\item Ant Colony Optimization Algorithm - 

\item Honey Bee Foraging Algorithm - 
...
In order to examine whether a certain level of reliability we need to define how we measure reliability. 

One usual measurements for hardware reliability is Mean-Time Between Failure, MTBF which is defined as 
...
If we consider the case when a application is duplicated to secondary hardware the reliability function above become
$$R_{tot}(t) = 1 - (1 - e^{-t/MTBF_1})(1 - e^{-t/MTBF_2})\cdots(1 - e^{-t/MTBF_n})$$
for $n$ replicas. 
