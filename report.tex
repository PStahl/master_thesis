...
When nodes fail more often, there is a higher probability of nodes failing before it finishes to handle a \emph{lost node} request, or to replicate one of its replicas. As described in~\cref{sec:replication_time}, this results in a higher time $t_R$. This is why the average is higher, and there are more times above 100 ms and 500 ms, and the 95th percentile value, in the experiment with \emph{MTBF} of 10 seconds, than the experiment with \emph{MTBF} of 30 seconds.
	...
	\begin{tabular}{| c | c | c | c | c |}
	 ...
	 \hline
	  ...
	  10 s & 107.3 & 597 & 34 & 194.5 \\ %times the actual exceeded 115
	   \hline
	\end{tabular}
	 \caption[Average $t_R$, number of times above 100 ms and 500 ms and the 95th percentile in~\cref{sec:eval_time_t}]{Average time $t_R$, number of times above 100 ms and 500 ms, and the 95th percentile value for \emph{MTBF} 30 and 10 in~\cref{sec:eval_time_t}}
	 \label{table:tr}
	 \end{center}
 \end{table} 
