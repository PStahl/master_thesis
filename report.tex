...
As previously mentioned, reliability will be ensured by the use of replication. More specifically, active replication is used, where each replica receives the same input, and performs the same calculations. \Cref{fig:app_model_replication} shows how the application in \cref{fig:app_model} looks after replicating task $S$ 4 times. It is also possible to have several services replicated, this scenario is shown in \cref{fig:extended_app_model}.
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
	\Require{$current\_nodes$ is a list of nodes currently holding a replica, and $R_{req}$ is the reliability to fulfill}
	\Statex
	\Procedure{Greedy scheduling algorithm}{}
	\State $nodes\gets $ \Call{get available nodes}{$current\_nodes$}
	\While {\Call{reliability}{$current\_nodes$} $\leq R_{req}$}
...
\begin{algorithm} 
	...
	\begin{algorithmic}[1]
	\Require{$current\_nodes$ is a list of nodes currently holding a replica, and $R_{req}$ is the required reliability}
	\Statex
	...
	\State $least\_reliable\gets current\_nodes.pop$
	\While {\Call{reliability}{$current\_nodes$}\ >\ $R_{req}$}
		\State $replica\gets $\Call{replica at}{$least\_reliable$}
		\State
		\Call{delete replica from node}{$replica$, $least\_reliable$}
		\State $least\_reliable\gets current\_nodes.pop$
...
For the various state sizes, an actor was started on one of the two runtimes and replicated to the other runtime 10 times. For three state sizes, 1, 50 and 100 MB, we measured the replication time 100 times and calculated the standard deviation of these values.
...
%TODO
 ...
 \begin{table}[h!]
	...
	\begin{tabular}{| c | c | c | c | c |}
	 \hline
	 X & state size (MB) & average (s) & STD & STD (\%) \\
	 \hline		%TODO values
	  server & 1 & 0.041 & 0.001 & 2.442 \\
	  server & 50 &  5.523 &  0.056 & 1.012\\
	  server & 100 &  20.177 & 0.385 & 1.908 \\
	  laptop & 1 & \\
	  laptop & 50 & \\
	  laptop & 100 & \\
	   \hline
	\end{tabular}
	 \caption{Average replication time, mean value and standard deviation for state sizes 1, 50 and 100 MB, where the size was increased by increasing the size of one of the actor's variables.}
	 \label{table:rep_mean_deviation_variable}
	 \end{center}
 \end{table}
...
\begin{table}[h!]
	...
	\begin{tabular}{| c | c | c | c | c |}
	 ...
	 X & state size (MB) & average (s) & STD & STD (\%) \\
	 \hline		%TODO values
	  server & 1 & 0.251 & 0,008 & 3.166 \\
	  server & 50 & 16,547 & 0.256 & 1.550 \\
	  server & 100 & 42.555 & 0.414 & 0.972 \\
	  laptop & 1 & \\
	  laptop & 50 &  \\
	  laptop & 100 &  \\
	   \hline
	\end{tabular}
	 \caption{Average replication time, mean value and standard deviation for state sizes 1, 50 and 100 MB, where the size was increased by increasing the size of the actor's port queue.}
	 \label{table:rep_mean_deviation_queue}
	 \end{center}
 \end{table}
...
\begin{itemize}
	...
	\item \Cref{fig:replication_time_less_than_40} the measure replication times for state sizes less than 40 MB.
...
\caption{Worst case scenario for detecting a failed node. In this example, node A fails directly after sending the heartbeat. The actual time node A has been dead before it's detected by node B is therefore close to 500 ms.} \label[app]{fig:heartbeats_worst_case}
\end{figure}

\begin{figure}[!hbt]
\centering
\includegraphics[scale=0.5]{images/heartbeats_best_case.pdf}
\caption{Best case scenario for detecting a failed node. In this example, node A fails just before sending the third heartbeat. The actual time node A has been dead before it's detected by node B is therefore close to 300 ms.} \label[app]{fig:heartbeats_best_case}
\end{figure}
