...
\subsection{Replication} \label{ch:background_replication}
...
Unlike \cite{designFaultTolerantSched}  \cite{evalReplicationSched} \cite{taskSchedulingReplication} \cite{effTaskReplMobGrid} \cite{relGridServicePredConstraint} we propose a fully dynamic model which monitors the system and create new replicas in order to ensure that the desired reliability is met. This is achieved by monitoring both how many replicas as operational, on which nodes they are executing, as well as node failures, in order to dynamically adapt our reliability model as the properties of the system varies.

The rest of this chapter is composed as follows, in \ref{sec:limitations} we go through what limitations we have done. \ref{sec:system_model} gives a description of the system model we have used, e.g. in which computational environment we test our model, how we model the faults and their distribution and what kind of applications we use. In \ref{sec:reliability_model} we present our reliability model, which is our main contribution with this paper. We state start by giving diverse definitions and a scheduling algorithm and then go through our self-adapting model. Last in this chapter, \ref{sec:implementation} we briefly describe how Calvin works and present the main parts of the implementation of our model.
...
A small example of four interconnected nodes  is shown in figure~\ref{fig:computational_environment}. We refer to a computational resource as a \emph{node}. Due to the lack of access to a cluster consisting of heterogenous hardware components, the cluster used the experiments consists of homogenous hardware components, and varying behaviour in terms of failure is simulated in the experiments. This if further described in chapter ~\ref{ch:evaluation}.

Since all nodes are interconnected we have redundant paths which results in that all nodes are directly reachable even if a node fails. In Appendix \ref{appendix:A} a system of one replicated actor is shown before and after a node failure.
...
In contrast to the case with one \emph{primary} replica and several \emph{secondary} replicas, as described in \ref{ch:background_replication}, we will adapt a fan-in fan-out model, where all replicas both receive the same input, but also all transmit its result to the consumer. %TODO do we have to have this? I don't think it really affects our model right?
...
\section{Reliability model} \label{sec:reliability_model}
...
\begin{algorithmic}\label{algo:scheduling}
\State $n\gets 0$
...
\subsection{Our contributions} % ish?
...
\section{Computational environment} \label{eval:comp_env}
We mentioned in ~\ref{ch:design}, we implemented our model using Calvin. In order to evaluate our model, we set up an execution environment consisting of 6 servers with homogeneous hardware components. A calvin runtime was started on each of these servers. One of these runtimes was stable, while each of the other runtimes were periodically killed and restarted, simulating node failures. 

The reason for having a stable runtime was to have the consumer deployed to this runtime, while the producers were deployed to the failing runtimes. The application if further described in ~\ref{eval:application}.

\section{Application} \label{eval:application}
...
\section{Measurement of time $t$}
As mentioned in \ref{sec:time_t}, the time $t$ is the time it takes from that a node on which a replica dies, until a new replica is up and running, and depends on the time it takes to detect that the node died, and the time it takes to create a new replica.

\subsection{Measurement of node failure detection time}
% Wrong? No heartbeat
Because of Calvin's use of DHT with automatic node discovery, nodes automatically detect when other nodes fail, by the use of heartbeats, explained in ~\ref{sec:monitoring}.
...
\subsection{Measurement of replication time}
...
\begin{appendices}
\chapter{} \label{appendix:pictures}
