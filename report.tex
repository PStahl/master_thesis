...
Our model is implemented by extending the actor-based application environment \emph{Calvin} ~\cite{calvin}, developed by Ericsson. While \emph{Calvin} is mainly an environment for IoT applications, it suites our purpose well. The model is evaluated by... %TODO

The report is structured as follows: in Section ~\ref{ch:background} all necessary background theory is provided, in Section ~\ref{ch:design} we present our model and contribution in more detail, Section ~\ref{ch:evaluation} aims at evaluating our solution while Section ~\ref{ch:future_work} presents future work. Section ~\ref{ch:conclusions} concludes the report. 
...
\subsection{Dynamic versus static environments} \label{background_dyn_stat_env}
...
\subsection{Failure assumptions} \label{subsec:background_failure_assump}
...
\subsection{Factors affecting reliability} \label{subsec:background_factors}
...
\iffalse
\subsection{Fault life-cycle techniques} \label{subsec:background_fault_life_cycle}
...
%TODO
Virtualization is a broad term of creating a virtual version of something, e.g. a server, a hard drive or a private network (VPN). The wins of virtualization is partly savings in hardware cost and space and partly in that the system becomes more dynamic. E.g. it is possible to virtualize a hundred servers on fifty physical servers since they usually have capacity left. Another advantage with virtualization is that it separates the software from the hardware which enables the possibility of moving a virtual machine from one physical server to another.
...
Compare with container - With virt. a machine running three virt. systems would have a hypervisor and three seperate operation systems running on to of it. Isolated from each other. With containers there's only one operating system and the three containers shares the systems kernel. [http://www.cio.com/article/2924995/enterprise-software/what-are-containers-and-why-do-you-need-them.html]
...
Choosing streaming services with deterministic processing\footnote{A deterministic function always produces the same output given a certain input.} allows us to avoid timing issues and cases when the consumer receives different results. However, since using active replication the consumer will receive a result from each replica and our model could therefore easily be extended with consensus algorithms such as those presented in ~\ref{subsub:consensus} to determine whether or not the correct result was received. 
...
The rest of this chapter is composed as follows, in ~\ref{sec:design_limitations} we go through what limitations we have done. \ref{sec:design_system_model} gives a description of the system model we have used, e.g. in which computational environment we test our model, how we model the faults and their distribution and what kind of applications we use. In ~\ref{sec:design_reliability_model} we present our reliability model, which is our main contribution with this paper. We start by giving diverse definitions and a scheduling algorithm and then go through our self-adapting model. Last in this chapter, ~\ref{sec:design_implementation} we briefly describe how Calvin works and present the main parts of the implementation of our model.

\section{Limitations} \label{sec:design_limitations}
As described in ~\ref{ch:TODO} we limit our model to only consider node failures, i.e. we assume that links between nodes can not fail. However, the 	reliability model could easily be extended to include link failure probabilities, but then they have to be taken into account in the scheduling algorithm as well which would have required a lot of more work on the scheduling algorithm.
...
By doing these limitations we avoid unnecessary complexity in form of checkpointing and consensus. The impact of this will be discussed later in report ~\ref{ch:discussion}.
...
In this paper we assume that all resources are within the same cluster, with low latency connections between them. The resources consist of heterogenous hardware with high bandwidth low latency reduntant links between them.

A small example of four interconnected nodes  is shown in figure~\ref{fig:computational_environment}. We refer to a computational resource as a \emph{node}. Due to the lack of access to a cluster consisting of heterogenous hardware components, the cluster used for experiments consists of homogenous hardware components, and varying behaviour in terms of failure is simulated in the experiments. This if further described in chapter ~\ref{ch:evaluation}.

Since all nodes are interconnected we have redundant paths which results in that all nodes are directly reachable even if a node fails. In Appendix~\ref{appendix:figures} a system of one replicated actor is shown before and after a node failure.
...
All information, such as how many replicas are currently executing, and on which nodes they are running, must be globally available. If stored on a single node, e.g. the executing node, that information is lost in the case of failure. Also, if a remote database is used, we introduce a single-point-of-failrue. Instead, to achieve a redundant storage, Distributed Hash Table (DHT) is used, which efficiently avoids having a single-point-of-failure. The DHT used is further explained in ~\ref{sec:calvin_storage}.

%Skip reference to Calvin storage? It only says 'not explained in the thesis'
...
In this paper we use a simple example application in our experiments. The application can be modelled as shown in figure~\ref{fig:app_model}. The node $A$ shown in the figure could for example be a service for which we require a certain reliability. No assumptions are made about the execution time of the application. However, our model is particularly beneficial for long running applications and services, as they in large-scale distributed environments are required to stay operational even in the case of unpredictable failures ~\cite{imprRelAdaptRL}.
...
As previously mentioned, reliability will be ensured by the use of replication. More specifially, active replication will be used where each replica receives the same input, and performs the same calculations. Figure~\ref{fig:app_model_replication} shows how the application in figure~\ref{fig:app_model} looks after replicating task $A$ 4 times.

In contrast to the case with one \emph{primary} replica and several \emph{secondary} replicas, as described in ~\ref{subsec:background_replication}, we will adapt a fan-in fan-out model, where all replicas both receive the same input, but also all transmit its result to the consumer. %TODO do we have to have this? I don't think it really affects our model right?
...
Unlike the reliability models presented in XXX, we do not consider link failures in our model. However, in our experiments, link failures are indeed possible, but in the case of a link failure a node will be assumed dead, and a new replica created. Therefore, the desired reliability will still be met. %TODO we should have something here about links right? 
...
However, when a system first is set up and started, no knowledge about the failure rates of the system is known. Therefore, when the system is set up, we do start with assuming constant failure rates. As time goes and the time of failure for nodes are stored, one will get a more precise value for the MTBF for the nodes in the system. The time it takes to detect node failure and spawn a new replica is further discussed in ~\ref{sec:design_time_t}.
...
A small example is presented in figure~\ref{fig:handling_node_failure}. In the situation shown in the figure, there are 4 connected nodes, 1 to 4, and the other nodes have just detected the failure of node 4. Nodes 1 to 4 will select the node with the highest ID, in this case node 3, which will be responsible for determining which actions to take.
...
\subsubsection{Node monitor} %Or "Heartbeat"
...
%TODO make sure the number are correct (two heartbeats = node failure)
...
We have also added an application monitor on each node that once every five seconds checks the current reliability for all actors in the applications started on the node. % Mention any time at all? Five seconds??
...
We mentioned in ~\ref{ch:design}, we implemented our model using Calvin. In order to evaluate our model, we set up an execution environment consisting of 6 servers with homogenous hardware components. A calvin runtime was started on each of these servers. One of these runtimes was stable, while each of the other runtimes were periodically killed and restarted, simulating node failures. 

The reason for having a stable runtime was to have the consumer deployed to this runtime, while the producers were deployed to the failing runtimes. The application if further described in ~\ref{sec:eval_application}.
...
The computational environment and the application is shown in figure~\ref{fig:evaluation}.

%REPLICATION TIME:
%An average of the last "self.history" (default is 5) times. We consider these values to be most relevant. 

\section{Measurement of time t} \label{sec:eval_time_t}
As mentioned in ~\ref{sec:design_time_t}, the time $t$ is the time it takes from that a node on which a replica dies, until a new replica is up and running, and depends on the time it takes to detect that the node died, and the time it takes to create a new replica.

\subsection{Measurement of node failure detection time} \label{subsec:eval_node_fail_time}
% Wrong? No heartbeat
Because of Calvin's use of DHT with automatic node discovery, nodes automatically detect when other nodes fail, by the use of heartbeats, explained in ~\ref{subsec:design_monitoring}.
...
The average of the 50 measured times was $483.8 ms$. This corresponds to the theoretical average of $500 ms$. %TODO this experiment is not needed if we set a timeout of 500ms instead of increasing a count everytime we send a heartbeat and assume lost when we have sent 3 without receiving a reply. Atm, sending every 200ms and assuming dead when count > 2, the average case is 500 ms. But using a timeout one can have a deterministic timeout.
...
\subsubsection*{Result}
Figure ~\ref{fig:replication_time} shows how the replication time vary depending on the state size. The replication time is clearly linear in relation to the size of the state.
...
\section{Reliability level} \label{sec:eval_rel_level}
...
\section{Optimal number of replicas} \label{sec:eval_opt_nbr_replicas}
...
When using a fail-stop fault model, we assume that when a node dies all other nodes are aware of this. This is clearly limiting, as in the case of a link failure, a node, $A$ may become unavailable for one node $B$ while being available for another $C$. Assume that we have one replica on $A$ and one on $B$. Both node $A$ and $B$ will assume that the other has failed and tell the node with highest id to replicate its actors. If $C$ has the highest id of all three then nothing will happen since $C$ is aware of the replicas on both $A$ and $B$. But if at least one of $A$ and $B$ has higher id than $C$ it will send the replication request to itself and since it's not aware of that there is a running replica on the other node it will replicate the actor to $C$. Thereafter we have an unnecessary high reliability and, from a system perspective, there is a unnecessary high impact on the network load.
...
There are a lot of different kinds of failures which could be taken into account, see ~\ref{future:extended_model} for an extended reliability model.
...
In our experiments we have had only one actor to replicate on the lost node, in bigger systems with many actors on each node we get the complexity of choosing which actor to replicate first. A solution could be to have a criticality of each actor type and start replicating the most critical actor on the lost node. However, replication time is defined as the time it takes from that we start replicating actors at all after a node failure to that the actor is up and running, i.e. all actors replication times have the same start time. This will result in that the least critical actor will have the largest replication time and thereby being having the most amount of replicas. %Is this a bad thing? I mean, the least ciritical actor will use a lot of network (bad) but since it is low prioritized after a node failure made it's ok?
Another, perhaps better solution would be to replicate the actors with smallest states first in order to minimize each actor's replication time, kind of a earliest deadline first algorithm. We leave the investigation of which algorithm is the optimal to future work.
...
\subsection{Extended model} \label{future:extended_model}
...
In this appendix a system consisting of one actor is shown before and after a node fails. The required reliability is 0.995 and for a certain replication time and failure rate the nodes reliability is given under their name.
