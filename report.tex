...
The reliability model used in this thesis is fairly simple. Firstly, it requires a node to have failed at least twice before knowing its \emph{MTBF}, otherwise a default value is used. In case the default value is higher than the actual, it results in the calculated reliability of a node is actually lower than the actual. Furthermore, despite adapting a node's \emph{MTBF} by using only the three latest failure times, it takes a few failures before the calculated \emph{MTBF} correctly reflects the node's behavior. Imagine a node failing once every week. After some time, it for some reason starts failing every hour. Then the calculated \emph{MTBF} for that node will be one hour first after failing three times. Until then, the calculated \emph{MTBF} will be higher, and thereby the reliability higher than the actual. Since periodically monitoring the reliability however, some machine learning technique would be beneficial to try predict the reliability of nodes.

To get a node's failure times, its failures must be detectable. This is achieved using the heartbeat system described in~\cref{subsec:heartbeats}. If the application consists of several tasks being replicated, as shown in~\cref{fig:extended_app_model}, the heartbeat system will no longer be necessary. Assume we have two tasks A and B, with replicas $A_1$, $A_2$ and $B_1$, $B_2$, where $A_1$ and $A_2$ both send their results to $B_1$ and $B_2$. If both $B_1$ and $B_2$ stop receiving results from $A_1$ or $A_2$, they can together make a decision whether or not to the node has failed. Furthermore, if $A_1$ and $A_2$ stop receiving acknowledgments of received messages from $B_1$ or $B_2$, they can assume it has failed.

Furthermore, when using a fail-stop fault model (see \cref{subsub:background_fail_stop}), we assume that when a node fails, all other nodes are aware of this, since they will no longer receive heartbeats. However, the assumption of a fully reliable network with redundant paths is obviously limiting. In case of a link failure in a real setting, it could happen that some nodes will not receive heartbeats from a node, while others will. The nodes not receiving the heartbeat will then falsely assume the node has died. Such a situation may result in new replicas being created, perhaps more than necessary, and thereby putting an unnecessarily high demand on the system in terms of resource usage. Although, since the optimization algorithm in run periodically, the unnecessary replicas will eventually be removed.

We further assume tasks always will produce the correct result. Since the receiving task will receive a result of each replica, it would be easy to implement some consensus algorithm such as \emph{majority voting} or \emph{k-modular redundancy} at the receiver. Thereby, one could adapt the broader Byzantine fault model instead of the fail-stop model.
...
With the primary objective being to ensure a certain level of reliability, and doing so using active replication, we require a lot more resources, which puts an extra burden on the system. In addition, the extra load on the system may affect the execution time, thus decreasing task performance.
