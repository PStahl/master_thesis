...
All information, such as how many replicas are currently executing, and on which nodes they are running, must be globally available. If stored on a single node, e.g. the executing node, that information is lost in the case of failure. Also, if a remote database is used, we introduce a single-point-of-failrue. Instead, to achieve a redundant storage, Distributed Hash Table (DHT) is used, which efficiently avoids having a single-point-of-failure. The DHT used is further explained in~\autoref{sec:calvin_storage}.
...
\subsection{Extended model} \label{subsec:future_extended_model}
...
For measure reliability according to the dynamic definition (\autoref{TODO}) we will use the following formula:
% Jag tror inte vi har en dynamisk definition
