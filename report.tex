...
\subsection{Consensus} \label{sub:consensus}
...
% ... among the various fault-tolerant techniques, our model focus on fault tolerance. Failures are assumed to happen, and appropriate actions should be taken when they happen. Proactive technique.
...
Choosing streaming services with deterministic processing\footnote{A deterministic function always produces the same output given a certain input.} allows us to avoid timing issues and cases when the consumer receives different results. However, since using active replication the consumer will receive a result from each replica and our model could therefore easily be extended with consensus algorithms such as those presented in \cref{sub:consensus} to determine whether or not the correct result was received. 
