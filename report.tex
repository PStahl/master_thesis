...
%TODO
Virtualization for cloud - What makes virtualization so important for the cloud is that it decouples the software from the hardware. [dummies.com] 
...
The time it takes to replicate a task, depends on the size of the task state, which have to be sent to the new node. In this experiment, we incrementally increase the state of a task, by increasing the size of its input and output queue, and measured the time it took to replicate.
...
Figure ~\ref{fig:replication_time} shows how the replication time vary depending on the state size.

\begin{figure}[!hbt]
\centering
%\includegraphics[scale=2.5]{images/results/replication_time.pdf} 
