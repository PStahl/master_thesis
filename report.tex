...
Grid computing is the collection of autonomous resources that are distributed geographically and by a single domain work together to achieve a common goal, i.e. to solve a single task \cite{A Comparative Study of Load Balancing Algorithms in Cloud Computing Environment}.

Clusters are similar to grid but with the difference that they aren't distributed geographically. They work in parallel under supervision of a single administrative domain. From the outside it looks like a single computing resource. \cite{A Comparative Study of Load Balancing Algorithms in Cloud Computing Environment}
...
In dynamic environment the cloud provider installs heterogeneous resources. The resources are  exible in dynamic environment. In this scenario cloud cannot rely on the prior knowledge whereas it takes into account run-time statistics. \cite{A Comparative Study of Load Balancing Algorithms in Cloud Computing Environment}
...
%TODO 
In \cite{A large-scale study of failures in high-performance computing systems}, real-world failure data is studied and in that data, hardware failures was the single most common type of failure, ranging from 30 to more than 60\%.
...
%TODO
Previous work usually assumes resource failure within a system follows a Poisson process \cite{Distributed workflow mapping algorithm for maximized reliability under end-to-end delay constraint} \cite{Matching and Scheduling Algorithms for Minimising Execution Time and Failure Probability of Applications in Heterogeneous Computing} \cite{Reliability model of distributed simulation system} \cite{Optimal task allocation for maximizing reliability in distributed real-time systems} \cite{Performance Implications of Periodic Checkpointing on Large-scale Cluster Systems}, with a constant failure rate. For this model to be valid, it is assumed that failures between resources are statistically independent \cite{Distributed workflow mapping algorithm for maximized reliability under end-to-end delay constraint}. 
...
Assumptions such as constant resource failure rate and statistically independent failures, are not likely to model the actual failure scenario of a dynamic heteregenous distributed system \cite{Matching and Scheduling Algorithms for Minimising Execution Time and Failure Probability of Applications in Heterogeneous Computing}. 
\cite{A survey on reliability in distributed systems} show that chances of hardware failure are greater in the beginning and end of lifetime. However, by studying failures in high-performance computing systems, \cite{A large-scale study of failures in high-performance computing systems} found a Poisson distribution to be a poor fit, while a normal or lognofmrla distribution fitted the data much better.
...
We focus on the study of task allocation deci- sion to achieve maximal distributed system reliability (DSR) under processor resource constraints and total system cost constraint. the task allocation problem has been studied to achieve various goals, such as reliabil- ity maximization, safety maximization, fault tolerance increasing, and cost minimization. Their processor reliability was com- puted from the processor failure rate and the elapsed execution time. As far as we know, the effects of module software reliabilities and module execution frequencies on the optimal task allocation decision have not been studied before. To pursue this topic, we di- vide the processor reliability into two parts in our system model: the processor hardware reliability and the module software reliability [The decision model of task allocation for constrained stochastic distributed systems]
...
\bibitem{Distributed workflow mapping algorithm for maximized reliability under end-to-end delay constraint}
	Distributed workflow mapping algorithm for maximized reliability under end-to-end delay constraint

\bibitem{Matching and Scheduling Algorithms for Minimising Execution Time and Failure Probability of Applications in Heterogeneous Computing}
...
\bibitem{Reliability model of distributed simulation system}
Reliability model of distributed simulation system

\bibitem{Optimal task allocation for maximizing reliability in distributed real-time systems}
Optimal task allocation for maximizing reliability in distributed real-time systems

\bibitem{Performance Implications of Periodic Checkpointing on Large-scale Cluster Systems}
Performance Implications of Periodic Checkpointing on Large-scale Cluster Systems

\bibitem{A survey on reliability in distributed systems}
A survey on reliability in distributed systems

\bibitem{A Comparative Study of Load Balancing Algorithms in Cloud Computing Environment}
